...
import {
  addSalesUnit as onAddSalesUnit,
  deleteSalesUnit as onDeleteSalesUnit,
  getSalesUnit as onGetSalesUnit,
  updateSalesUnit as onUpdateSalesUnit
} from "../../store/salesUnit/actions";
...
function Main() {
  ...
  const { salesUnit, loading, error, message, addSuccess, updateSuccess, deleteSuccess } =
    useSelector((state) => state.SalesUnitReducer);
    ...
    useFormik({
      ...
      onSubmit: (values, { resetForm }) => {
        ...
        if (basicSlideOverPreview.type === "Add") {
          dispatch(onAddSalesUnit(values));
        }
  ...
  const handleDeleteChange = () => {
    dispatch(onDeleteSalesUnit(deleteModalPreview.data.id));
  };
  ...
  return (
    <>
      <div className="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 className="text-lg font-medium mr-auto">Unit Of Measure</h2>
      </div>
        ...
        <div className="intro-y box p-5 mt-5 ">
          <div className="overflow-x-scroll">
            <div className="" style={{ minWidth: "1000px" }}>
              <ReactTabulator
                columns={columns}
                data={salesUnit?.docs ? salesUnit.docs : []}
                options={optionTab}
              />
            </div>
          </div>
          ...
          <div className="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center mt-6">
            <nav className="w-full sm:w-auto sm:mr-auto">
              <ul className="pagination">
                  ...
                  <>
                    ...
                    <li className="page-item">
                      <a
                        className="page-link"
                        href="#"
                        onClick={() => setCurrentPage(salesUnit?.prevPage)}
                      >
                ...
                {salesUnit?.hasNextPage ? (
                  <>
                    <li className="page-item">
                      <a
                        className="page-link"
                        href="#"
                        onClick={() => setCurrentPage(salesUnit?.nextPage)}
                      >
                        {salesUnit?.nextPage}
                      </a>
                    </li>
                    ...
                    <li className="page-item">
                      <a
                        className="page-link"
                        href="#123"
                        onClick={() => setCurrentPage(salesUnit?.totalPages)}
                      >
