...
import { LoadingIcon, Lucide, Modal, ModalBody, ModalHeader } from "@/base-components";
import { ReactTabulator, reactFormatter } from "react-tabulator";
import {
  addSites as onAddSites,
  deleteSites as onDeleteSites,
  getSites as onGetSites,
  updateSites as onUpdateSites
} from "../../store/actions";
...
function Main() {
  ...
  const { Sites, loading, error, message, addSuccess, updateSuccess, deleteSuccess } = useSelector(
  ...
  useEffect(() => {
    if (basicSlideOverPreview.type === "Edit") {
      ...
      setFieldValue("company", {
        value: basicSlideOverPreview.data.company,
        label: basicSlideOverPreview.data.company
      });
    } else {
      resetForm();
    }
  ...
  const { values, errors, handleSubmit, handleChange, handleBlur, setFieldValue, resetForm } =
    useFormik({
      ...
      onSubmit: (values, { resetForm }) => {
        const body = {
          name: values.name,
          company: basicSlideOverPreview.data?.company ?? 1003 // TODO : Enter Company Id that is fetched by login
        };
  ...
  const handleDeleteChange = () => {
    dispatch(onDeleteSites(deleteModalPreview.data.id));
  };
  ...
  return (
    <>
      <div className="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 className="text-lg font-medium mr-auto">SITE</h2>
      </div>
                ...
                <>
                  ...
                  <li className="page-item">
                    <a
                      className="page-link"
                      href="#"
                      onClick={() => setCurrentPage(Sites?.prevPage)}
                    >
                ...
                <>
                  <li className="page-item">
                    <a
                      className="page-link"
                      href="#"
                      onClick={() => setCurrentPage(Sites?.nextPage)}
                    >
                      {Sites?.nextPage}
                    </a>
                  </li>
                  ...
                  <li className="page-item">
                    <a
                      className="page-link"
                      href="#123"
                      onClick={() => setCurrentPage(Sites?.totalPages)}
                    >
      ...
      >
        <ModalHeader className="p-5">
          <h1 className="font-medium text-lg mr-auto">
            {basicSlideOverPreview.type === "Edit" ? "Edit Site" : "Add New Site"}
          </h1>
        </ModalHeader>
        <ModalBody>
          <Fragment>
